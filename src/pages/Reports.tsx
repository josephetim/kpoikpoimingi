...
const Reports = ({ role }: ReportsProps) => {
  ...
  const contracts = (contractsQuery.data ?? []).filter((contract) => visibleBranches.includes(contract.branch));
  const payments = (paymentsQuery.data ?? []).filter((payment) => visibleBranches.includes(payment.branch));
  ...
  const collectionsByBranch = BRANCHES.filter((branch) => visibleBranches.includes(branch)).map((branch) => ({
    branch,
    value: payments
  ...
  const outstandingByBranch = BRANCHES.filter((branch) => visibleBranches.includes(branch)).map((branch) => ({
    branch,
    value: contracts
  ...
  const assetTypeSummary = ITEM_CATEGORIES.map((assetType) => ({
    assetType,
    count: contracts.filter((contract) => contract.category === assetType).length,
  }));

  const overdueSummary = contracts
    .map((contract) => ({
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Card title="Total Contracts" value={String(contracts.length)} />
        <Card title="Total Outstanding Balance" value={formatCurrency(totalOutstanding)} />
        <Card title="Total Collections" value={formatCurrency(payments.reduce((sum, payment) => sum + payment.amount, 0))} />
        <Card title="Overdue Contracts" value={String(overdueSummary.length)} action={<button type="button" className="btn-secondary text-xs" onClick={exportSummaryCsv}>Export Summary CSV</button>} />
      </div>
